...
class SampleGenerator:
    """
    Генерация сэмплов с правильной длиной периода для XM.

    XM воспроизводит сэмпл с rate 8363 Hz при ноте C-4 (note 49, relative_note=0).
    Частота C-4 = 261.63 Hz.
    Один период волны = 8363 / 261.63 ≈ 32 сэмпла.

    Если сэмпл содержит ОДИН период длиной 32 сэмпла и зациклен,
    то при ноте C-4 он будет звучать на 261.63 Hz — правильно.

    Для качества звука генерируем несколько периодов (кратно 32),
    но loop_length = один период = 32 сэмпла.
    """

    PERIOD = 32  # сэмплов на один период при C-4 / 8363 Hz

    @classmethod
    def square_wave(cls, duty=0.5):
        p = cls.PERIOD
        return [24000 if (i % p) < int(p * duty) else -24000 for i in range(p)]
    ...
    def triangle_wave(cls):
        ...
        for i in range(p):
            phase = (i % p) / p
            if phase < 0.25:
                s.append(int(96000 * phase))
            elif phase < 0.75:
                s.append(int(24000 - 96000 * (phase - 0.25)))
            else:
                s.append(int(-24000 + 96000 * (phase - 0.75)))
        return s

    @classmethod
    def sawtooth_wave(cls):
        p = cls.PERIOD
        return [int(48000 * (i % p) / p - 24000) for i in range(p)]

    @staticmethod
    def noise(length=4096):
        import random
        random.seed(42)
        return [random.randint(-24000, 24000) for _ in range(length)]

    @classmethod
    def fm_organ(cls):
        p = cls.PERIOD * 2  # больше сэмплов для FM качества
        s = []
        for i in range(p):
            ph = 2 * math.pi * i / p
            s.append(int(math.sin(ph + math.sin(ph * 2) * 1.5) * 24000))
        return s
    ...
    def fm_brass(cls):
        ...
        for i in range(p):
            ph = 2 * math.pi * i / p
            v = math.sin(ph + math.sin(ph * 3) * 2.0) * 0.8 + math.sin(ph * 2) * 0.2
            s.append(int(v * 24000))
        return s
    ...
    def fm_bass(cls):
        ...
        for i in range(p):
            ph = 2 * math.pi * i / p
            s.append(int(math.sin(ph + math.sin(ph * 0.5) * 3.0) * 24000))
        return s
    ...
    def fm_lead(cls):
        ...
        for i in range(p):
            ph = 2 * math.pi * i / p
            v = math.sin(ph + math.sin(ph * 4) * 1.0) * 0.7 + math.sin(ph * 3) * 0.3
            s.append(int(v * 24000))
        return s

    @classmethod
    def fm_strings(cls):
        p = cls.PERIOD * 4  # больше гармоник
        s = []
        for i in range(p):
            ph = 2 * math.pi * i / p
            v = (math.sin(ph) * 0.5 + math.sin(ph * 2.01) * 0.25 +
                 math.sin(ph * 3.01) * 0.15 + math.sin(ph * 4.01) * 0.1)
            s.append(int(v * 24000))
        return s
    ...
    def fm_piano(cls):
        ...
        for i in range(p):
            ph = 2 * math.pi * i / p
            mod = math.sin(ph * 14) * 0.5 * math.exp(-i / p * 2)
            s.append(int(math.sin(ph + mod) * 24000))
        return s
...
class XMWriter:
    ...
    def _build_instruments(self, transpose=0, finetune=0):
        ...
        def mk(name, pcm, loop=True):
            """
            Создание инструмента с правильным loop.
            pcm — один или несколько периодов волны.
            Для зацикленных: loop_start=0, loop_length=len(pcm)
            (весь сэмпл = один период, зациклен).
            """
        ...
        # FM инструменты генерируются с кратной длиной (PERIOD*N),
        # поэтому для них relative_note нужно скорректировать.
        # Базовый PERIOD=32 → C-4 при 8363 Hz. Верно.
        # PERIOD*2=64 → частота в 2 раза ниже → на октаву ниже → relative_note += 12
        # PERIOD*4=128 → на 2 октавы ниже → relative_note += 24

        p = SampleGenerator.PERIOD

        def mk_fm(name, pcm, periods):
            """FM инструмент с коррекцией для кратного периода"""
        ...
        self.instruments = [
            mk("PSG Square 50%", gen.square_wave(0.5)),       # 1
            mk("PSG Square 25%", gen.square_wave(0.25)),      # 2
            mk("PSG Square 12%", gen.square_wave(0.125)),     # 3
            mk("PSG Noise", gen.noise(), loop=False),          # 4
            mk_fm("FM Organ", gen.fm_organ(), 2),              # 5
            mk_fm("FM Brass", gen.fm_brass(), 2),              # 6
            mk_fm("FM Bass", gen.fm_bass(), 2),                # 7
            mk_fm("FM Lead", gen.fm_lead(), 2),                # 8
            mk_fm("FM Strings", gen.fm_strings(), 4),          # 9
            mk_fm("FM Piano", gen.fm_piano(), 2),              # 10
            mk("AY Square A", gen.square_wave(0.5)),           # 11
            mk("AY Square B", gen.square_wave(0.25)),          # 12
            mk("AY Triangle", gen.triangle_wave()),            # 13
            mk_fm("OPM FM Lead", gen.fm_lead(), 2),            # 14
        ]
